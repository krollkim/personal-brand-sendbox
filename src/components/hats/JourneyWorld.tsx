...
export default function JourneyWorld({
  ...
  // Initial placement only. When this world is part of the desktop choreography
  // the pair is walked ALONG the trail by the Hats timeline (scroll-driven, via
  // the data-journey-* hooks) so the motion is smooth and tied to the scroll —
  // no self-running loop (a loop would "teleport" back to the start each cycle).
  ...
  return (
    <div className="relative h-[100svh] w-full overflow-hidden bg-white">
      ...
      <div className="absolute right-[6%] top-1/2 z-10 flex max-w-md -translate-y-1/2 flex-col items-start text-right sm:right-[8%]">
        ...
        <p className={`mt-4 max-w-sm text-base leading-relaxed sm:text-lg ${cBody}`}>{hat.body_he}</p>
      </div>
    </div>
  );
}
