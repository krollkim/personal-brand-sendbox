...
import { WORK_TITLE, WORK_SUBTITLE, PROJECTS, type WorkType } from "@/lib/work";

gsap.registerPlugin(ScrollTrigger, useGSAP);

// Outlined category chips — colour ties each project back to a hat.
const TYPE_STYLE: Record<WorkType, string> = {
  בנייה: "border-purple/35 text-purple-deep",
  תוכן: "border-fuchsia/40 text-fuchsia",
  ליווי: "border-purple-soft/50 text-purple-soft",
};

function Mockup({ ratio = "aspect-[4/3]" }: { ratio?: string }) {
  return (
    <div
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-sm tracking-[0.3em] text-ink-muted/50">[תצוגה]</span>
      </div>
    </div>
  );
}

function Meta({ name, type, oneLiner }: { name: string; type: WorkType; oneLiner: string }) {
  return (
    <div className="text-right">
      <span
        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${TYPE_STYLE[type]}`}
      >
        {type}
      </span>
  ...
  return (
    <section ref={root} id="work" className="relative bg-white py-28 sm:py-36">
      <div className="mx-auto max-w-6xl px-6">
        {/* header */}
        <div className="mb-14 text-right sm:mb-20">
          ...
          <p className="mt-4 text-lg text-ink-muted sm:text-xl">{WORK_SUBTITLE}</p>
        </div>
        ...
        >
          <Mockup ratio="aspect-[16/10]" />
          <Meta name={featured.name} type={featured.type} oneLiner={featured.oneLiner} />
        </article>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {rest.map((pr) => (
            <article key={pr.name} data-card className="group flex flex-col gap-4">
              <Mockup />
              <Meta name={pr.name} type={pr.type} oneLiner={pr.oneLiner} />
            </article>
          ))}
        </div>
      </div>
